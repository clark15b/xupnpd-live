...
namespace libjson
{
    ...
    int lua_json_pdecode(lua_State* L);

    int lua_json_encode_value(lua_State* L,std::string* buf,int index);

    int luaL_pack_utf8_to_unicode(std::string* buf,const unsigned char* p,int len);
    int luaL_pack_unicode_to_utf8(luaL_Buffer* buf,const unsigned char* p);

    void luaL_addjsonlstring(std::string* buf,const char* p,size_t l);
...
{
    lua_register(L,"json_encode",libjson::lua_json_encode);

    lua_register(L,"json_decode",libjson::lua_json_pdecode);

    lua_register(L,"json_no_unicode_escape",libjson::lua_json_no_unicode_escape);
...
{
    if(lua_gettop(L)>0)
        no_unicode_escape=lua_tointeger(L,1);

    return 0;
}

int libjson::luaL_pack_utf8_to_unicode(std::string* buf,const unsigned char* p,int len)
{
    char tmp[32];
...
void libjson::luaL_addjsonlstring(std::string* buf,const char* p,size_t l)
{
...
int libjson::lua_json_encode_value(lua_State* L,std::string* buf,int index)
{
    ...
    case LUA_TNUMBER:
        ...
        if(p)
            ...
            buf->append(l_null,sizeof(l_null)-1);

        break;
    case LUA_TBOOLEAN:
        if(lua_toboolean(L,index)>0)
            buf->append(l_true,sizeof(l_true)-1);
        else
            buf->append(l_false,sizeof(l_false)-1);
...
int libjson::lua_json_pdecode(lua_State* L)
{
